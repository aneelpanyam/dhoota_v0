-- ============================================
-- 047: Add type "date" to activity_date column in bulk activity table
-- So the QuestionCardWidget renders a date picker instead of text input.
-- Also fix SQL to handle empty activity_date (NULLIF avoids cast error on '').
-- ============================================

-- Handle empty activity_date in SQL
...
SELECT $1, $2,
  ...
  COALESCE((nullif(trim(coalesce(e->>''activity_date'','''')), ''''))::timestamptz, now()),
  e->>''location''
